...
**The recipe (proven across Phases 1–4; mechanized into a harness).** For anything that must reflect the *real* Electron process (main + preload + contextBridge, not just the renderer's HTML/CSS):
1. The Vite dev server must already be running — the unpackaged dev app loads `http://localhost:5173`. Never start or kill it yourself.
2. Use **`tools/cdp/harness.mjs`** — plain ESM, zero dependencies, not part of the pnpm workspace. Its `launchApp()` bakes in the pnpm bin-location gotcha (`./apps/desktop/node_modules/.bin/electron apps/desktop`, not the repo root's `node_modules/.bin`), the `--remote-debugging-port`, and BOTH isolation dirs (`COPILOT_BUDGET_DB_PATH` env for a fresh SQLite DB, `--user-data-dir` for a fresh PAT store — it has no env override; skip either and state leaks across runs, a real bug class caught in Task 3.1), discovers the page's WebSocket target from `http://127.0.0.1:<port>/json/list`, connects via Node ≥22's global `WebSocket`/`fetch` (no `ws` dependency), and enables the Runtime/Log/Page domains. The returned handle exposes `evaluate()` (boundary proofs, `getComputedStyle` fidelity), `screenshot()`, `consoleErrors()`, `boundaryProof()`, and `close()`.
3. Write a **throwaway** `.mjs` probe script per task, in a scratchpad dir, importing the harness — the harness is mechanism-only and asserts nothing itself; every selector, expected value, and pass/fail judgment belongs in the probe. Probes are never committed, so assertions can't rot against a moving UI.
4. Tear down every run: `close()` kills the Electron process and removes both temp dirs (idempotent) — call it in a `finally` so a failed assertion still cleans up.
