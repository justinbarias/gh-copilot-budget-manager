...
**"Chrome MCP" against the Electron app, specifically — read as "interactive CDP verification."** The second half of the §6.7 gate is real CDP verification of the actual Electron process, not the `mcp__chrome-devtools__*` / claude-in-chrome browser-MCP tool suites. Those tools drive their *own* Chromium instance — pointing one at `http://localhost:5173` opens a bare browser tab with no Electron main process behind it, so `contextBridge`-exposed APIs (`window.api`) are absent and any IPC-backed data never arrives. This is invisible for a static placeholder (Task 0.2) but wrong from Phase 1 onward, once screens depend on preload-delivered data. Reserve those MCP tools for contexts with no Electron main process in the loop (e.g. the future standalone `apps/web`); they are not the tool for this app.

**The recipe (proven across Phases 1–4).** For anything that must reflect the *real* Electron process (main + preload + contextBridge, not just the renderer's HTML/CSS):
1. The Vite dev server must already be running — the unpackaged dev app loads `http://localhost:5173`.
2. Launch the real binary directly, isolated from any other run: `./apps/desktop/node_modules/.bin/electron apps/desktop --remote-debugging-port=<port>` (pnpm only symlinks the `electron` bin into `apps/desktop`'s own `node_modules/.bin`, not the repo root's), with `COPILOT_BUDGET_DB_PATH` pointed at a fresh temp dir (isolates the SQLite DB) and `--user-data-dir=<fresh tmpdir>` (isolates the PAT store — it has no env override; skip this and PAT state leaks across runs, a real bug class caught in Task 3.1).
3. Discover the page's WebSocket target from `http://127.0.0.1:<port>/json/list`, then connect a raw WebSocket using Node ≥22's global `WebSocket`/`fetch` — no `ws` dependency needed. Write throwaway `.mjs` driver scripts in a scratchpad dir; never commit them.
4. Drive it via CDP domains: `Runtime.evaluate` for boundary proofs (`process === undefined`, `require === undefined`, `window.api`'s shape — contextIsolation/sandbox/bridge held) and for exact design-token fidelity (`getComputedStyle`); `Page.captureScreenshot` for visuals; the console/`Log` domains for the zero-app-level-errors assertion; the `Network` domain where relevant (e.g. proving staged edits issue zero mutations — main-process Octokit traffic correctly never appearing in the renderer's network log is itself PAT-isolation evidence).
5. Tear down: kill the Electron process you launched, remove the temp profile/DB dirs.

**Visual inspection is literal, not a figure of speech.** The screenshot → Read-tool step means the agent *looks at the pixels* (Claude is multimodal) — pair eyes with calipers: screenshots for layout/unmistakability judgment calls, `getComputedStyle` for exact token values. Do both; neither substitutes for the other. Canonical example of why this pass exists and can't be skipped: the Task 2.5 sim-banner bug, where Playwright's `toBeVisible()` passed while the banner had scrolled fully out of the viewport — only the CDP scroll + screenshot + a human look caught it (a §6.8 violation, fixed same-day).

Playwright e2e (headless `_electron.launch()` against MSW) remains the automated, blocking half of the gate; interactive CDP verification is the second half — the human-eyes check headless can't make. Both green, or it isn't done.
