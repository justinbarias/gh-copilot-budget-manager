...
- [x] 9.2 Live read smoke — shape reconciliation, §6.9 rows upgraded to "confirmed against live", MSW corrected [M] (closed 2026-07-12: read surface `live-confirmed` on the real tenant via the R1–R7 smoke arcs of 2026-07-08→11, MSW realigned, OpenAPI file pinned as the §6.9 source. Narrow remainders stay tracked in `docs/api-surface-validation.md` "Task 9.2 reconcile checklist": cap block-vs-overflow field name (#1), budget-list pagination reader (#6, inert on this single-page tenant), A1 `X-OAuth-Scopes` pin (#10), two R5 placeholder values (#12).)
- [x] 9.3 Live-write arming flow (was: + read/write token separation + RBAC-lite roles) [M] — **descoped by maintainer ruling 2026-07-12:** for the 2–3-trusted-admin desktop audience, the 9.3-lite arming gate (typed enterprise-slug confirmation, main-process-memory flag, disarm-on-relaunch, force-disarm on credential/tenant change) is the FINAL separation-of-duties answer; read/write token separation and RBAC-lite roles are dropped, not deferred. 9.3-lite itself validated 2026-07-09 (see sub-item below).
