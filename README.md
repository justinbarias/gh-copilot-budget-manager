...
>
> **Scope ruling (maintainer, 2026-07-12) — arming is the final answer.** Read/write token *separation* and RBAC-lite roles from the original Task 9.3 charter are **descoped, not deferred**: one PAT does both reads and writes, and for this tool's 2–3-trusted-admin desktop audience the explicit arming gate above (plus disarm-on-relaunch and force-disarm on credential/tenant change) is the accepted separation-of-duties model. What was "9.3-lite" — the in-app mode toggle and the arming gate — is therefore the complete 9.3.
